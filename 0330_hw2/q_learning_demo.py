...
def q_learning(epsilon_start=0.5):
    ...
    for episode in range(EPISODES):
        ...
        while not done:
            ...
            if not done:
                max_future_q = np.max(q_table[ns])
...
def run_and_save(epsilon_start=0.5):
    q_table, info, metrics = q_learning(epsilon_start=epsilon_start)
    ...
    out_dir = Path(__file__).resolve().parent / "pic" / "q_learning"
    out_dir.mkdir(exist_ok=True)
    suffix = f"{epsilon_start:.1f}"
    curves_path = out_dir / f"q_learning_curves_eps_{suffix}.png"
    policy_path = out_dir / f"q_learning_policy_blocks_eps_{suffix}.png"
