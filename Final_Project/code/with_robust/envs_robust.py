...
ENV_CONFIGS: Dict[str, Dict[str, object]] = {
    ...
    "5": {
        "name": "env_5_mixed_validation",
        "env_urdf": ENV_URDF_DIR / "env_5.urdf",
        "robot_base_pos": [0.05, -0.06, 1.0225],
        "env_base_pos": [0.0, 0.0, 1.0],
        "target_x": None,
        "success_margin": 0.05,
...
DEFAULT_FRICTION_RANDOMIZATION_RANGES: Dict[str, Tuple[float, float]] = {
...
DEFAULT_SENSOR_NOISE_STD = {
    # Default observation noise for the robust version. All angular terms are in rad or rad/s.
    # This is intentionally stronger than the previous conservative setting so the effect is
    # visible in the report plots and meaningful for robustness training.
    "joint_angle": np.deg2rad(1.0),
    "joint_velocity": np.deg2rad(5.0),
    "base_position": 0.005,
    "base_euler": np.deg2rad(1.0),
    "base_linear_velocity": 0.02,
    "base_angular_velocity": np.deg2rad(5.0),
    "gripper_position": 0.005,
    "gripper_euler": np.deg2rad(1.0),
    "gripper_linear_velocity": 0.02,
}
...
class LedgeClimbEnv(gym.Env):
    """Gymnasium environment for ledge climbing with optional robustness perturbations."""
    ...
    def __init__(
        ...
        use_friction_randomization: bool = False,
        friction_randomization_ranges: Optional[Dict[str, Tuple[float, float]]] = None,
        use_sensor_noise: bool = False,
        sensor_noise_std: Optional[Dict[str, float]] = None,
        use_observation_filter: bool = False,
        observation_filter_alpha: float = 0.8,
    ):
        super().__init__()
        ...
        self.use_friction_randomization = bool(use_friction_randomization)
        self.friction_randomization_ranges = dict(
            DEFAULT_FRICTION_RANDOMIZATION_RANGES
            if friction_randomization_ranges is None
            else friction_randomization_ranges
        )
        self.use_sensor_noise = bool(use_sensor_noise)
        self.sensor_noise_std = dict(DEFAULT_SENSOR_NOISE_STD if sensor_noise_std is None else sensor_noise_std)
        # Kept for backward compatibility only. In the robust PPO pipeline,
        # the environment returns noisy observations and the PPO script applies
        # the low-pass filter before observation normalization.
        self.use_observation_filter = bool(use_observation_filter)
        self.observation_filter_alpha = float(np.clip(observation_filter_alpha, 0.0, 1.0))
        self.obs_filter_state: Optional[np.ndarray] = None
        self.current_friction_params: Dict[str, float] = {}
        self.last_sensor_noise = np.zeros(1, dtype=np.float32)
        self.last_sensor_noise_mean_abs = 0.0
        self.last_sensor_noise_max_abs = 0.0
        self.last_clean_obs = np.zeros(1, dtype=np.float32)
        self.last_noisy_obs = np.zeros(1, dtype=np.float32)
        ...
        # Sensor block = joint q/qd + base pose/velocity + gripper pose/velocity.
        # Previous residual, reference phase, and current reference are not sensor-noised.
        self.sensor_obs_dim = (2 * self.n_joints) + 12 + 12 + 6
        self.sensor_noise_std_vector = self._build_sensor_noise_std_vector()
        self.last_sensor_noise = np.zeros(self.sensor_obs_dim, dtype=np.float32)
        self.last_clean_obs = np.zeros(self.obs_dim, dtype=np.float32)
        self.last_noisy_obs = np.zeros(self.obs_dim, dtype=np.float32)
    ...
    def set_env_id(self, env_id: str) -> None:
        """Switch to another ledge URDF before the next reset."""
        env_id = str(env_id)
        if env_id not in ENV_CONFIGS:
            raise ValueError(f"Unknown env_id={env_id!r}. Available ids: {list(ENV_CONFIGS)}")
        self.env_id = env_id
    ...
    def _range_sample(self, key: str) -> float:
        low, high = self.friction_randomization_ranges[key]
        if high < low:
            low, high = high, low
        return float(self.np_random.uniform(float(low), float(high)))

    def _build_sensor_noise_std_vector(self) -> np.ndarray:
        std = np.zeros(self.obs_dim, dtype=np.float32)
        idx = 0
        std[idx:idx + self.n_joints] = float(self.sensor_noise_std.get("joint_angle", 0.0)); idx += self.n_joints
        std[idx:idx + self.n_joints] = float(self.sensor_noise_std.get("joint_velocity", 0.0)); idx += self.n_joints
        std[idx:idx + 3] = float(self.sensor_noise_std.get("base_position", 0.0)); idx += 3
        std[idx:idx + 3] = float(self.sensor_noise_std.get("base_euler", 0.0)); idx += 3
        std[idx:idx + 3] = float(self.sensor_noise_std.get("base_linear_velocity", 0.0)); idx += 3
        std[idx:idx + 3] = float(self.sensor_noise_std.get("base_angular_velocity", 0.0)); idx += 3
        std[idx:idx + 3] = float(self.sensor_noise_std.get("gripper_position", 0.0)); idx += 3
        std[idx:idx + 3] = float(self.sensor_noise_std.get("gripper_euler", 0.0)); idx += 3
        std[idx:idx + 3] = float(self.sensor_noise_std.get("gripper_position", 0.0)); idx += 3
        std[idx:idx + 3] = float(self.sensor_noise_std.get("gripper_euler", 0.0)); idx += 3
        std[idx:idx + 3] = float(self.sensor_noise_std.get("gripper_linear_velocity", 0.0)); idx += 3
        std[idx:idx + 3] = float(self.sensor_noise_std.get("gripper_linear_velocity", 0.0)); idx += 3
        return std

    def _apply_observation_noise_and_filter(self, obs: np.ndarray) -> np.ndarray:
        """Return the environment observation after sensor noise only.

        Sensor-noised block layout:
          joint angles, joint velocities, base position/euler/velocities,
          gripper positions/eulers/linear velocities.

        The last blocks are intentionally left clean:
          previous residual action, reference phase, current reference action.

        Low-pass filtering is intentionally not applied here in the robust PPO script.
        The PPO script filters the noisy observation before RunningMeanStd
        normalization, which keeps the simulator/environment as the source of
        noisy measurements and the policy code as the estimator/filter stage.
        """
        obs = obs.astype(np.float32, copy=True)
        sensor_slice = slice(0, self.sensor_obs_dim)
        self.last_sensor_noise = np.zeros(self.sensor_obs_dim, dtype=np.float32)
        self.last_sensor_noise_mean_abs = 0.0
        self.last_sensor_noise_max_abs = 0.0

        if self.use_sensor_noise:
            noise = self.np_random.normal(0.0, self.sensor_noise_std_vector[:self.sensor_obs_dim]).astype(np.float32)
            obs[sensor_slice] += noise
            self.last_sensor_noise = noise.copy()
            self.last_sensor_noise_mean_abs = float(np.mean(np.abs(noise)))
            self.last_sensor_noise_max_abs = float(np.max(np.abs(noise)))

        return obs

    def _set_dynamics_for_all_links(self, body_id: int, **kwargs: float) -> None:
        if body_id is None:
            return
        for link_idx in range(-1, p.getNumJoints(body_id, physicsClientId=self.client_id)):
            p.changeDynamics(body_id, link_idx, physicsClientId=self.client_id, **kwargs)

    def _apply_robust_domain_randomization(self) -> None:
        self.current_friction_params = {}
        if not self.use_friction_randomization:
            return
        env_lat = self._range_sample("env_lateral_friction")
        robot_lat = self._range_sample("robot_lateral_friction")
        spinning = self._range_sample("spinning_friction")
        rolling = self._range_sample("rolling_friction")
        self.current_friction_params = {
            "env_lateral_friction": env_lat,
            "robot_lateral_friction": robot_lat,
            "spinning_friction": spinning,
            "rolling_friction": rolling,
        }
        if self.env_body_id is not None:
            self._set_dynamics_for_all_links(
                self.env_body_id,
                lateralFriction=env_lat,
    ...
    def _load_scene(self) -> None:
        ...
        self._apply_robust_domain_randomization()
    ...
    def reset(self, seed: Optional[int] = None, options: Optional[dict] = None):
        ...
        if options is not None and "env_id" in options:
            self.set_env_id(str(options["env_id"]))
        self.step_counter = 0
        self.reference_step = 0
        self.prev_residual_action = np.zeros(self.n_joints, dtype=np.float32)
        self.obs_filter_state = None
    ...
    def _get_obs(self) -> np.ndarray:
        ...
        obs = np.concatenate(
            [
                ...
                obs_reference_action.astype(np.float32),
            ],
            axis=0,
        ).astype(np.float32)
        ...
        self.last_clean_obs = obs.copy()
        obs = self._apply_observation_noise_and_filter(obs)
        self.last_noisy_obs = obs.copy()
        return obs
    ...
    def _get_joint_debug_data(self) -> Tuple[list[str], np.ndarray, np.ndarray, np.ndarray]:
        assert self.robot_id is not None
        ...
        joint_torques = np.asarray([state[3] for state in states], dtype=np.float32)
        return joint_names, joint_angles, joint_velocities, joint_torques
    ...
    def _get_info(
        ...
        joint_names, joint_angles, joint_velocities, joint_torques = self._get_joint_debug_data()
        energy_cycle_metrics = self._get_energy_and_cycle_metrics()
        return {
            "env_id": self.env_id,
            "use_friction_randomization": bool(self.use_friction_randomization),
            "use_sensor_noise": bool(self.use_sensor_noise),
            "use_observation_filter": bool(self.use_observation_filter),
            "observation_filter_alpha": float(self.observation_filter_alpha),
            "sensor_obs_dim": int(self.sensor_obs_dim),
            "sensor_noise_mean_abs": float(self.last_sensor_noise_mean_abs),
            "sensor_noise_max_abs": float(self.last_sensor_noise_max_abs),
            "sensor_noise_std": dict(self.sensor_noise_std),
            "friction_params": dict(self.current_friction_params),
...
if __name__ == "__main__":
    print("envs_robust_v4.py path:", Path(__file__).resolve())
